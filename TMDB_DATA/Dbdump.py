import requests
import json

# TMDb API 키
api_key = '4df7ab026552454751d28aad32ea63aa'

# 가져올 페이지 수
num_pages = 500

# 각 페이지에서 가져올 결과 수
results_per_page = 20

# 영화 데이터를 저장할 리스트
movie_data = []

# 페이지 반복
for page in range(1, num_pages + 1):
    # TMDb API 엔드포인트 URL
    url = 'https://api.themoviedb.org/3/movie/popular?include_adult=true'
    # url = 'https://api.themoviedb.org/3/discover/movie?include_adult=true&include_video=false&language=ko&sort_by=vote_average.desc'
    params = {
        'api_key': '4df7ab026552454751d28aad32ea63aa',
        'region': 'KR',
        'language': 'ko',
        'page': page
    }

    # 영화 정보 요청
    response = requests.get(url, params=params)

    # 요청이 성공했을 경우
    if response.status_code == 200:
        page_data = response.json()
        results = page_data['results']

        # 각 영화 결과를 리스트에 추가
        movie_data.extend(results)

        print(f"페이지 {page}의 영화 정보를 가져왔습니다.")
    else:
        print(f"페이지 {page}에서 영화 정보를 가져오는데 실패했습니다.")

# JSON 파일로 저장
with open('movie_data.json', 'w', encoding='utf-8') as file:
    json.dump(movie_data, file, indent=4, ensure_ascii=False)

print("모든 영화 정보가 성공적으로 저장되었습니다.")
